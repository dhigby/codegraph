...
- CodeGraph now indexes **Keyman** keyboard source (`.kmn`) — groups become callable units, user and system stores become symbols, and `use(...)` calls plus `outs`/`any`/`index` store references are wired into the graph, so you can trace how a keystroke flows through a keyboard's groups and which stores each group consumes.
- Keyman support now spans the whole **keyboard ecosystem**, not just `.kmn`: touch/mobile layouts (`.keyman-touch-layout`), package manifests (`.kps`), visual keyboards (`.kvks`), and catalog metadata (`.keyboard_info`) are all indexed. A keyboard's files are linked into one **cross-file subgraph** — the package manifest connects to its `.kmn`, which connects to its touch layout and visual keyboard — so you can ask which files make up a keyboard and how they relate. Touch layouts expose each layer and its `nextlayer` transitions; `.kps` and `.keyboard_info` record which human languages each keyboard supports (so *"which keyboards support language X"* is answerable across both modern and legacy keyboards), and packages are cross-linked to their related packages.
